...
/**
 * Publica <b>uma</b> linha da Outbox no Kafka, cada uma na sua transação — assim
 * uma linha "envenenada" não impede as seguintes. Sucesso ⇒ {@code published_at};
 * falha ⇒ {@code attempts++} + {@code last_error} (a linha volta na próxima rodada).
 *
 * <p>Entrega "pelo menos uma vez": se o {@code send} vai ao Kafka mas o commit do
 * {@code published_at} falha, a linha é republicada — daí a idempotência no consumidor (Etapa 2).
 */
@Slf4j
@Component
...
public class OutboxDispatcher {
    ...
    public void dispatch(Long id) {
        ...
        if (event == null || event.getPublishedAt() != null) {
            return; // já publicado por outra rodada / gatilho pós-commit
        }
